...
\presentadd[\authfn{2}]{School of Biological Sciences, Queen's University Belfast, BT9 7BL Belfast, United Kingdom}
...
\afterpage{%
	\begin{tcolorbox}
		...
		It should be noted, that even though "missingness" for low-abundant proteins and peptides is more common than for higher abundant peptides (i.e. an issue related to the dynamic range and sensitivity of the mass spectrometer), this problem can also arise due to fundamental peptide interference regardless of their abundance or the acquisition parameters. 
		...
		Much more importantly, imputed values may also bias the analysis past the batch effect adjustment stage. As shown in Fig ~\ref{fig:batch_fig4_missing_values}B and C, if re-quantifications ("re-quants", values inferred from MS elution traces) are used, the correlation within batches seems higher than the correlation of replicates, while this problem is not observed when imputation is not used ("no re-quants"). Protein inference is also affected by the imputation on lower levels.
	...
	\vspace{-0.5cm}	\captionof{figure}[l]{\textbf{The problem of missing values in batch effect diagnosis and correction: Aging mouse study} }
